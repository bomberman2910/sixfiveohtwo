...
pub const Cpu = struct {
    ...
    pub fn clock(self: *Cpu) !void {
        ...
        const source: OperandSource = switch (decoded.b) {
            0 => switch (decoded.c) {
                0 => switch (decoded.a) {
                    0, 2, 3 => .{ .is_implied = true },
                    1 => .{ .operand_address = try self.readAddressFromProgramCounter() },
                    5, 6, 7 => .{ .immediate_operand = try self.readNextByte() },
                    else => return CpuError.illegal_opcode,
                },
                1 => .{ .operand_address = try self.getZeroPageXIndirectAddress() },
                2 => switch (decoded.a) {
                    5 => .{ .immediate_operand = try self.readNextByte() },
                    else => return CpuError.illegal_opcode,
                },
                else => return CpuError.illegal_opcode,
            },
            1 => switch (decoded.c) {
                0 => switch (decoded.a) {
                    ...
                    else => .{ .operand_address = try self.readNextByte() },
                },
                1, 2 => .{ .operand_address = try self.readNextByte() },
                else => return CpuError.illegal_opcode,
            },
            2 => switch (decoded.c) {
                0 => .{ .is_implied = true },
                1 => switch (decoded.a) {
                    ...
                    else => .{ .immediate_operand = try self.readNextByte() },
                },
                2 => switch (decoded.a) {
                    0...3 => .{ .is_ac = true },
                    4...7 => .{ .is_implied = true },
            ...
            3 => switch (decoded.c) {
                0 => switch (decoded.a) {
                    ...
                    else => .{ .operand_address = try self.readAddressFromProgramCounter() },
                },
                1, 2 => .{ .operand_address = try self.readAddressFromProgramCounter() },
                else => return CpuError.illegal_opcode,
            },
            4 => switch (decoded.c) {
                0 => .{ .immediate_operand = try self.readNextByte() },
                1 => .{ .operand_address = try self.getZeroPageYIndirectAddress() },
                else => return CpuError.illegal_opcode,
            },
            5 => switch (decoded.c) {
                0 => switch (decoded.a) {
                    4, 5 => .{ .operand_address = try self.getZeroPageXAddress() },
                    else => return CpuError.illegal_opcode,
                },
                1 => .{ .operand_address = try self.getZeroPageXAddress() },
                2 => switch (decoded.a) {
                    4, 5 => .{ .operand_address = try self.getZeroPageYAddress() },
                    else => .{ .operand_address = try self.getZeroPageXAddress() },
                },
                else => return CpuError.illegal_opcode,
            },
            6 => switch (decoded.c) {
                0 => .{ .is_implied = true },
                1 => .{ .operand_address = try self.getAbsoluteYAddress() },
                2 => switch (decoded.a) {
                    4, 5 => .{ .is_implied = true },
                    else => return CpuError.illegal_opcode,
                },
            ...
            7 => switch (decoded.c) {
                0 => switch (decoded.a) {
                    5 => .{ .operand_address = try self.getAbsoluteXAddress() },
                    else => return CpuError.illegal_opcode,
                },
                1 => .{ .operand_address = try self.getAbsoluteXAddress() },
                2 => switch (decoded.a) {
                    4 => return CpuError.illegal_opcode,
                    5 => .{ .operand_address = try self.getAbsoluteYAddress() },
                    else => .{ .operand_address = try self.getAbsoluteXAddress() },
                },
                else => return CpuError.illegal_opcode,
            },
        };
        ...
        switch (decoded.a) {
            ...
            4 => switch (decoded.c) {
                ...
                1 => try self.bus.write(operand.?.address, self.state.ac),
                2 => switch (decoded.b) {
